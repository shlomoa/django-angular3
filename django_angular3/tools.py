...
def get_download_url(release_data, os_name, arch):
    ...
    raise RuntimeError(f"Could not find a suitable oasdiff binary for {os_name} {arch}.")
...
def ensure_oasdiff():
    ...
    try:
        ...
        if not oasdiff_path.exists():
            raise RuntimeError(f"Extraction completed, but {exe_name} was not found in {BIN_DIR}.")
