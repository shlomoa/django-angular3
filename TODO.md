...
| OpenAPI bootstrap command (`openapi-setup --inputPath`, `npm install`, `npm run generate:api`) | `build_app` already emits `openapi-setup` build-plan steps; add a standalone djng `openapi-setup` wrapper and align the bootstrap workflow (`--inputPath`, install, `generate:api`). | Pending |
