...
| `ng_gen_app [path]` | Generate an Angular application inside the configured workspace via the `angular-django2:material-app` schematic (forwards `--ssr`, `--zoneless`, and `--defaults`). Accepts `--app-name <name>`. |
...
| `ng_gen_app [path]` | Generate an Angular application via the `angular-django2:material-app` schematic (forwards `--ssr`, `--zoneless`, and `--defaults`). |
