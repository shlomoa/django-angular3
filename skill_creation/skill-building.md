...
The highest-bandwidth form here is a sample: an example input, a hand-written "good" output, or an existing spec doc. Much better than describing in prose. `doc/contracts/AI_AUTOMATION_CONTRACTS.md` provides the canonical Skill contracts, `doc/SKILL_AUTHORING_PLAN.md` defines the authoring and verification cadence, and this folder provides derived working copies.
