...
coverage axes, together with canonical scenario definitions, inputs, and test
oracles.
...
## 7. Canonical scenario definitions and test oracles
...
snapshots, provider-adapter boundary, and coverage axes are defined in sections
2–6 of this document. Project-configuration discovery and baseline resolution
are defined in [SPECIFICATIONS.md] §2.2. The examples below retain only
scenario inputs and expected outcomes.

Automation sequencing is owned by the [Automation Plan]. Scenario-test work
and acceptance coverage are owned by the [Verification Plan]; this section
...
invokes all 11 skills in dependency order. Baseline for verifying the complete
automation chain.
...
18. *(verification)* — terminal verification command (per
    ...
    structured outputs of the TOOL commands above
...
Suite invocation and the required coverage matrix are specified in sections 3
