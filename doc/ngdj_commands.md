...
| `ng generate angular-django2:site` | Assemble a validated OpenUI-defined Material site in an existing app. | Create/modify uses `--source` (OpenUI definition) or `--defaults` for the Home-only site; delete uses the ownership manifest with `--operation=delete --confirmDelete=true`. Also accepts `--project`, `--authGuard=authGuard`, `--csrfCookieName=csrftoken`, and `--csrfHeaderName=X-CSRFToken`. | `ng_site` | Wrapper; build_app pending | `docs/commands.md` |
| `ng generate angular-django2:material-setup --project=<name>` | Configure Angular Material in an existing project. | `--theme`, `--typography`, `--animations`. | `ng_material_setup` | Complete | `docs/commands.md`, `README.md` |
...
| Command | Purpose | Documented options | djng wrapper | Integration | djng docs |
| --- | --- | --- | --- | --- | --- |
| `ng generate angular-django2:component <name>` | Generate a standalone, `OnPush` component with embedding hooks. | Project-relative `--path`, `--project`, `--standalone=true`, `--changeDetection=OnPush`. | `ng_component` | Wrapper; build_app pending | `docs/commands.md`, `docs/workflow.md` §6, `README.md`, SKILL 07 |
| `ng generate angular-django2:page <name> --path=<path>` | Generate a standalone `OnPush` Material page with a feature-owned lazy route. | `--name` and `--path` are required; `--project`, `--routePath`, `--access=public`, `--authGuard=authGuard`, `--navigationLabel`, `--navigationIcon`. | `ng_page` | Wrapper; build_app pending | `docs/commands.md` |
...
| `ng generate angular-django2:field-component <name>` | Generate a simple façade over the canonical typed Material form field. | `--name` is required; `--path=src/app/shared/form-helpers`, `--project`, `--kind=text`. | — | Pending | Reference only |
| `ng generate angular-django2:form-field <name>` | Generate the canonical standalone `OnPush` typed `ControlValueAccessor` Material form field. | `--name` is required; `--path=src/app/shared/form-helpers`, `--project`, `--controlType=text`, `--appearance=fill`, `--subscriptSizing=fixed`. | — | Pending | Reference only |
| `ng generate angular-django2:reactive-form <name> --definition=<file>` | Generate a typed standalone `OnPush` Material reactive form from a single isolated JSON definition contract. | `--name` and `--definition` are required; `--path=src/app/features`, `--project`, `--primitivesPath=src/app/shared/form-helpers`. | `ng_reactive_form` | Wrapper; build_app pending | `docs/commands.md` |
