...
| `ng generate angular-django2:site` | Assemble a validated OpenUI-defined Material site in an existing app. | `--source` (OpenUI definition) or `--defaults=false` for the Home-only site; `--project`, `--operation=create`, `--confirmDelete=false`, `--authGuard=authGuard`, `--csrfCookieName=csrftoken`, `--csrfHeaderName=X-CSRFToken`. | — | Pending | Reference only |
...
| `ng generate angular-django2:page <name> --path=<path>` | Generate a standalone `OnPush` Material page with a feature-owned lazy route. | `--name` and `--path` are required; `--project`, `--routePath`, `--access=public`, `--authGuard=authGuard`, `--navigationLabel`, `--navigationIcon`. | — | Pending | Reference only |
...
| `ng generate angular-django2:reactive-form <name> --definition=<file>` | Generate a typed standalone `OnPush` Material reactive form from a single isolated JSON definition contract. | `--name` and `--definition` are required; `--path=src/app/features`, `--project`, `--primitivesPath=src/app/shared/form-helpers`. | — | Pending | Reference only |
